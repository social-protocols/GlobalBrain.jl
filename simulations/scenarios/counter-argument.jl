(sim::SimulationAPI) -> begin
    ...
    votes_A_step_1 = [
        SimulationVote(A.post_id, draw == 1 ? 1 : -1, i)
        for (i, draw) in enumerate(
            [supporters_draws_A_step_1; detractors_draws_A_step_1]
        )
    ]
    scores_step_1, _ = sim.step!(1, votes_A_step_1; description="There are $n_users users. Initially $(means["A"]*100)% agree with A")
    ...
    begin
        ...
        votes_B_step_2 = [
            SimulationVote(B.post_id, draw == 1 ? 1 : -1, i) 
            for (i, draw) in draws_B_step_2
        ]
        ...
        scores_step_2, _ = sim.step!(2, [votes_A_step_2; votes_B_step_2]; description="Among the $m users that consider B, $(means["A|B"]*100)% agree with A.")
    ...
    begin
        ...
        votes_C_step_3 = [
            SimulationVote(C.post_id, draw == 1 ? 1 : -1, i) 
            for (i, draw) in draws_step_3
        ]
    ...
    scores_step_3, _ = sim.step!(3, [votes_A_step_3; votes_C_step_3]; description="Among the $m users that also consider counter-argument C, agreement with A falls to $(round(means["A|B,C"]*100,digits=2))%.")
