...
TMPDIR=`mktemp -d /tmp/global-brain-service-test.XXXXXX`; (
    ...
    cat test-data/vote-events.jsonl| jq -s | jq -r '(["vote_event_id","user_id","parent_id","post_id","note_id","vote","vote_event_time"]) as $cols | map(. as $row | $cols | map($row[.])) as $rows | $cols, $rows[] | @csv' | tr -d '"' > $TMPDIR/vote-events.csv
    ...
    sqlite3 $TEST_DB_FILENAME -line ".eqp off" ".output $TMPDIR/tallies.txt" "select * from ConditionalTally order by post_id, note_id;";
